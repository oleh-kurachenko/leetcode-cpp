    ...
    void dfs(std::vector<std::vector<int>> &grid,
        ...
        auto process_node = [&](size_t next_x, size_t next_y) {
            if (grid[next_x][next_y] == 1) {
                ...
                if (f_up_grid[next_x][next_y] >= grid[x][y] &&
                    x_prev != S_MAX) {
                    min_connections = 1;
                }
                ++connections_count;
                ...
                f_up_grid[x][y] =
                    std::min(f_up_grid[x][y], grid[next_x][next_y]);
            }
        };
        ...
        if (n * m <= 2) {
            return std::accumulate(grid.begin(), grid.end(), 0,
                [](int sum, const std::vector<int> &line) {
                    return sum + std::accumulate(line.begin(), line.end(), 0);
                });
        }
        ...
        for (size_t i{0}; i < n; ++i) {
            for (size_t j{0}; j < m; ++j) {
                if (grid[i][j] == 1) {
                    ...
                    grid[i][j] = 2;
                    dfs(grid, f_up_grid, i, j, S_MAX, S_MAX);
                }
            }
        }

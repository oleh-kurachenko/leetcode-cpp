...
public:
    void append(int val) {
        entries.push_back(Entry{
            static_cast<uint16_t>(val), entries.back().multiplier,
            entries.back().addend
        });
    }
    ...
    int getIndex(int idx) {
        ...
        uint64_t effective_multiplier{
            entries.back().multiplier * get_mult_inverse(
                entries[idx].multiplier) % MODULO
        };

        uint64_t effective_addend{
            (entries.back().addend + MODULO - entries[idx].addend *
             effective_multiplier % MODULO) % MODULO
        };

        return static_cast<int>((entries[idx + 1].initial_value * effective_multiplier +
                effective_addend) % MODULO);
    }

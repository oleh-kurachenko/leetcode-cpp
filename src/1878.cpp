...
public:
    std::vector<int> getBiggestThree(std::vector<std::vector<int> > &grid) {
        ...
        for (size_t i{0}; i < grid.size(); ++i) {
            for (size_t j{0}; j < grid[0].size(); ++j) {
                for (size_t k{0}; i >= k && i + k < grid.size()
                                  && j + 2 * k < grid[0].size(); ++k) {
                    int sum = grid[i - k][j + k];

                    if (k) {
                        sum += l_diagonal_sum[i][j] - l_diagonal_sum[i - k][
                            j + k];
                        sum += r_diagonal_sum[i][j + 2 * k] - r_diagonal_sum[
                            i - k][j + k];
                        sum += r_diagonal_sum[i + k][j + k] - r_diagonal_sum[i][
                            j];
                        sum += l_diagonal_sum[i + k][j + k] - l_diagonal_sum[i][
                            j + 2 * k];
                        sum -= grid[i + k][j + k];
                    }

                    if (result.size() < RESULTS_COUNT || sum > *result.
                        begin()) {
                        result.insert(sum);

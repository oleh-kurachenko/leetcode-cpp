...
class Solution {
  private:
    ...
    TreeNode *build_tree(std::vector<int>::const_iterator preorder_begin,
        ...
        const auto inorder_root_position =
            inorder_map[*preorder_begin + NUMERIC_LIMIT + 1];
        const size_t left_size = inorder_root_position - inorder_begin;

        return new TreeNode(*preorder_begin,
            build_tree(preorder_begin + 1, preorder_begin + (left_size + 1),
                inorder_begin, inorder_root_position, inorder_map),
            build_tree(preorder_begin + (left_size + 1), preorder_end,
                inorder_root_position + 1, inorder_end, inorder_map));
    }

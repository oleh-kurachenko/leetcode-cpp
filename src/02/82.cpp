...
class Solution {
  private:
    ...
    void find_and_append_solutions(std::vector<std::string> &result,
        std::vector<int64_t> &digits, std::vector<char> &operations,
        ...
        for (size_t i{current_digit}; i < digits.size(); ++i) {
            ...
            find_and_append_solutions(result, digits, operations, i + 1,
                ...
                make_multiplication_group(current_number), target);
    ...
    void find_and_append_solutions(std::vector<std::string> &result,
        std::vector<int64_t> &digits, std::vector<char> &operations,
        size_t current_digit, int64_t current_addition_group,
        int64_t currenct_multiplication_group, int64_t target) {
        ...
        find_and_append_solutions(
            result, digits, operations, current_digit,
            [=](int64_t) {
                return current_addition_group + currenct_multiplication_group;
            },
            [](int64_t current_number) { return current_number; }, target);
        ...
        find_and_append_solutions(
            result, digits, operations, current_digit,
            [=](int64_t) { return current_addition_group; },
            [=](int64_t current_number) {
                return currenct_multiplication_group * current_number;
            },
            target);
  ...
  public:
    std::vector<std::string> addOperators(std::string num, int target) {
        ...
        find_and_append_solutions(
            solutions, digits, operations, 0, 0, 0, target);
        return solutions;
    }
};

...
private:
    ...
    long long countHeight(long long seconds, const std::vector<int>& workerTimes) {
        long long result{0};
...
public:
    long long minNumberOfSeconds(int mountainHeight, std::vector<int>& workerTimes) {
        ...
        while (true) {
            const auto current_height = countHeight(current_seconds, workerTimes);
            const auto previous_height = countHeight(current_seconds - 1, workerTimes);

            if (current_height >= mountainHeight && previous_height < mountainHeight) {
...
int main() {
    ...
    std::cout << Solution().minNumberOfSeconds(100000, workerTimes) << std::endl;
}